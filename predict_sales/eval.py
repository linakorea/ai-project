...
class SalesEvaluator:
    ...
    def _calculate_accuracy(self, actual: float, predicted: float) -> float:
        """
        정확도를 계산
        ...
        if actual == 0:
            return 0.0
        return (100 * (1 - abs(actual - predicted) / actual)).round(1)
    ...
    def evaluate(self) -> Tuple[pd.DataFrame, float]:
        ...
        Raises:
            ValueError: 데이터가 없거나 로드되지 않은 경우
        """
        ...
        eval_results['정확도(%)'] = eval_results.apply(
            lambda row: self._calculate_accuracy(row['실제값'], row['예측값']), 
            axis=1
        )
        ...
        total_accuracy = self._calculate_accuracy(total_actual, total_predicted)
...
def parse_year_month(year_month: str) -> Tuple[int, int]:
    ...
    Args:
        year_month: YYYYMM 형식의 문자열 (예: 202501)
    ...
    """
    if len(year_month) != 6:
        raise ValueError("입력은 YYYYMM 형식이어야 합니다 (예: 202501)")
...
def main():
    ...
    try:
        if len(sys.argv) != 2:
            print("사용법: python eval.py YYYYMM (예: python eval.py 202501)")
            sys.exit(1)
